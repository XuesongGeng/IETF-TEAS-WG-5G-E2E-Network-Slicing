...
The IETF network slice service is specified in terms of the set of SDPs connected to the slice, the type of connectivity among them, and a set of SLOs and SLEs for each connectivity construct.
...
{{?I-D.ietf-teas-nrp-scalability}} {{?I-D.ietf-teas-ns-ip-mpls}} and
 {{?I-D.ietf-teas-5g-ns-ip-mpls}}.
...
03333333, respectively.  Each instance of e2e network slice contains
...
specified endpoints (RAN/CN edge nodes). the IETF NSC might assign an IETF Network Slice
ID and send it to 3GPP NSMF.

8. The 3GPP NSMF could maintain the mapping relationship between S-NSSAI and
IETF Network Slice ID.
...
relationship between NSI identifier and IETF network slice ID.
...
the attributes with direct impact on the definition IETF network slice
profile.  It is a good start for selecting the transport network
...
3GPP NSMF will request the IETF Network Slice service by sending
the IETF Network Slice service request to the
IETF NSC.  The mapping relationship between NSI identifier and IETF
Network Slice identifier could be maintained in both 3GPP
NSMF and IETF NSC.
...
However, edge nodes between transport network and CN/AN may have IETF control plane protocol interactions, for example routing protocols.
...
SLE.  For example, {{Figure13}} could represent IETF network slice INS1 of {{Figure4}}
...
functions CU and UPF. {{Figure14}} shows this scenario.  Although the
...
respectively.  As shown in {{Figure14}}(A) and {{Figure14}}(B), the SDPs
...
corresponding IETF network slice.  As shown in {{Figure15}}, the IETF
...
IETF network slice. {{Figure16}} shows an example where the 5G E2E
...
### Mapping based on SRv6 SID
 ...
 SRv6 tunnels.  As shown in {{Figure17}}, the SRv6 SIDs is added by CU
...
Refer to section 4.2 of {{?I-D.ietf-teas-5g-ns-ip-mpls}} for details of this solution and how it is realized by provider network.
...
network slice traffic to IETF network slices.  As shown in {{Figure18}},
...
Once the PE nodes receives the IP packets, it may apply infer the context of the 5G E2E network slice and then apply a policy-
...
{: #Figure20 title="IETF Network Slice Controller NBI for 5G"}
   ...
   of 5G IETF network slices.  {{Figure21}} shows the mapping between NBI to
